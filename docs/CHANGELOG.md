...
### Fixed - Bug Fixes 🐛
**IV Rank Calculation:**
- Fixed options IV Rank calculation that was incorrectly using Historical Volatility instead of Implied Volatility
- Now fetches actual IV from Polygon options chain data
- Estimates 52-week IV range from historical volatility baseline (IV typically trades at 1.0-1.5x HV)
- Formula: `IV Rank = (Current IV - 52w Low IV) / (52w High IV - 52w Low IV) × 100`

**Agentic Brain Null Safety:**
- Fixed multiple `'NoneType' object has no attribute 'get'` errors in `src/ai/agentic_brain.py`
- Added null checks for `market_context` and `sector_context` before accessing properties
- All 5 unit tests now passing

**Scan Data Loading:**
- Fixed API loading wrong JSON file due to glob pattern matching `scans_index.json`
- Added filter to only match date-formatted scan files (`scan_YYYYMMDD_HHMMSS.json`)

**Test Suite:**
- Renamed `test_telegram_now.py` to `telegram_diagnostic.py` to prevent pytest crashes from `sys.exit(1)`

### Changed - Dashboard Simplification 🎨
**SEC Intel Tab (Simplified):**
- Removed: M&A Radar, Active Deals, SEC Alerts Feed, Pending Merger Filings table, Recent Awards
- Kept: Theme Spending, Patent Intelligence, Insider lookup
- Simplified search bar (Insider, Contracts, Patents buttons)

**Health Tab (Enhanced):**
- Added Economic Health hero zone with score gauge + key indicators (Yield Curve, Fed Rate, CPI Inflation)
- Added FRED stats bar (10Y/2Y Treasury, Unemployment, Jobless Claims, HY Spread, Consumer Sentiment)
- Added Full Economic Dashboard accordion
- Consolidated all economic/market health data in one tab

**Analytics Tab (Streamlined):**
- Removed Economic Health sections (moved to Health tab)
- Focused on: Technical Analysis, AI Briefing, Evolution Engine, Adaptive Weights

### Files Changed
- `src/analysis/options_flow.py` - IV Rank calculation fix
- `src/ai/agentic_brain.py` - Null safety fixes
- `modal_api_v2.py` - Scan file loading fix
- `tests/telegram_diagnostic.py` - Renamed from test_telegram_now.py
- `docs/index.html` - Dashboard restructure
